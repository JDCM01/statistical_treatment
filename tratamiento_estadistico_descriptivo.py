...
#standard deviation = raiz cuadrada de la varianza
#variance = suma de (dato - mean)^2 / numero de datos
...
def variance(data:list, number_of_data: int, mean: float):
    total = 0
    for i in data:
        total += (i - mean) ** 2
...
def main():
    ...
    statistical_variance = variance(data, number_of_data, mean)
    statistical_standard_deviation = statistical_variance ** 0.5
    statistical_mode = max(mode_count, key=mode_count.get)
    print(f"Mean: {mean}")
    print(f"Median: {statistical_median}")
    print(f"Mode: {statistical_mode}")
    print(f"Variance: {statistical_variance}")
    print(f"Standard Deviation: {statistical_standard_deviation}")
